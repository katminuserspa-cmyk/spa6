  ...
  try {
    ...
    invoicesData.forEach(inv => {
      ...
      if (inv.status === 'paid' && invDate.getMonth() === currentMonth && invDate.getFullYear() === currentYear) {
        revenueThisMonth += parseFloat(inv.total) || 0;
      } else if (inv.status === 'pending') {
        totalOutstanding += parseFloat(inv.total) || 0;
      }
    });

    container.innerHTML = `
      <div class="billing-analytics-cards d-flex gap-3 mb-4">
        <div class="card p-3 flex-fill bg-success text-white" style="border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
          <h5 class="mb-1">Revenue This Month</h5>
          <h3 class="mb-0">${utils.formatCurrency(revenueThisMonth, currency)}</h3>
        </div>
        <div class="card p-3 flex-fill bg-warning text-dark" style="border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
          <h5 class="mb-1">Total Outstanding</h5>
          <h3 class="mb-0">${utils.formatCurrency(totalOutstanding, currency)}</h3>
        </div>
      </div>
    ...
    <table>
      ...
      .map(
        (inv) => {
          ...
          const cgst = parseFloat(inv.cgst) || (tax / 2);
          const sgst = parseFloat(inv.sgst) || (tax / 2);
          ...
          <tr>
            ...
            <td>
              <div style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
                ...
                <select class="btn btn-sm" style="height: auto; padding: 4px 8px; margin: 0;" onchange="window.billingModule.updateStatus(${inv.id}, this.value)">
      ...
      const printHTML = `
        ...
        <head>
          ...
          <style>
            ...
            .invoice-container { max-width: 800px; margin: 0 auto; background: #fff; padding: 40px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border-radius: 8px; }
            ...
            .header-info h1 { margin: 0 0 5px 0; color: #1a1a1a; font-size: 28px; letter-spacing: -0.5px; }
            .header-info p { margin: 2px 0; color: #666; font-size: 14px; }
            .invoice-title { text-align: right; }
            .invoice-title h2 { font-size: 36px; color: #e9ecef; text-transform: uppercase; margin: 0; letter-spacing: 2px; }
            .details { display: flex; justify-content: space-between; margin: 30px 0; background: #f8f9fa; padding: 20px; border-radius: 6px; }
            .details p { margin: 5px 0; font-size: 14px; }
            .details strong { color: #1a1a1a; }
            table { width: 100%; border-collapse: collapse; margin: 30px 0; }
            th { background: #f8f9fa; padding: 12px 15px; text-align: left; font-weight: 600; color: #495057; border-bottom: 2px solid #dee2e6; text-transform: uppercase; font-size: 12px; letter-spacing: 0.5px; }
            td { padding: 15px; border-bottom: 1px solid #eee; font-size: 14px; color: #333; }
            .text-right { text-align: right; }
            .text-center { text-align: center; }
            .summary { margin: 30px 0 30px auto; width: 50%; }
            .summary-row { display: flex; justify-content: space-between; padding: 8px 15px; font-size: 14px; }
            .total-row { font-weight: bold; border-top: 2px solid #333; margin-top: 10px; padding-top: 15px; font-size: 16px; }
            .wallet-row { color: #e53935; background: #ffebee; border-radius: 4px; }
            .final-row { font-size: 22px; font-weight: 700; color: #1e88e5; margin-top: 15px; padding-top: 15px; border-top: 2px dashed #1e88e5; background: #f0f8ff; border-radius: 0 0 6px 6px; }
            .footer { margin-top: 60px; display: flex; justify-content: space-between; border-top: 1px solid #eee; padding-top: 30px; }
            .signature { text-align: center; width: 200px; }
            .signature p { margin: 5px 0; color: #666; font-size: 14px; }
            .signature-line { border-bottom: 1px solid #333; height: 40px; margin-bottom: 10px; }
            .note { margin-top: 40px; font-size: 13px; color: #888; text-align: center; font-style: italic; }
